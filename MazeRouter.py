...
def MazeRouter(inFile,outFile):
    ...
    for i in range(len(nets)):
        ...
        for j in range(len(nets[i])-1):
            tempList += findPath(nets[i][j][0],nets[i][j][1],nets[i][j][2],nets[i][j+1][0],nets[i][j+1][1],nets[i][j+1][2],grid) #every target becomes a source 
        ...
        if (len(tempList) != 0):
            ...
            print ('No possible path for net'+ str(i+1))
...
def HeuristicF(sourcePoint,targetPoint,parentL): 
    ...
    if LayerDist ==0 and (xDist!=0 or yDist !=0) and parentL==sourcePoint[2]: #special case where we have the two points on the same layer but we have to move horizontally on one layer and vertically on another layer, and use two vias
        zDist = 20
    else:
...
def findPath(L1,x1,y1,L2,x2,y2,grid):
    ...
    grid[x1][y1][L1].parent=Cell(-1,-1,-1,1)
    ...
    while True: #stops inside and returns the path once we meet the target, or breaks if there aren't any possible ways
        ...
        if grid[x_1][y_1][L_1-1].taken == 0 and L_1>1:
        ...
        if (len(currentCell.children) != 0): #if there are valid cells to move to next
            ...
            for cell in currentCell.children: # loop on the cells
                ...
                if cell.point[2] == currentCell.point[2]: #if the child cell is in the same layer
                    ...
                    if (cell.point[0]-parent.point[0]!=0 and cell.point[2]%2!=0) or (cell.point[1]-parent.point[1]!=0 and cell.point[2]%2==0):
            ...
            grid[parent.point[0]][parent.point[1]][parent.point[2]].children = [] #reset the children set of the parent (to avoid redundancy in future use)
            if currentCell.point[2] >=0 and currentCell.point[0]>=0 and currentCell.point[1] >=0 and (currentCell.point[2],currentCell.point[0],currentCell.point[1]) not in path:
            ...
            breakf=0
            if currentCell.point == (-1,-1,-1): #if we reach the starting point again, it means it faild to find a path
                breakf=1
...
MazeRouter('input file 3.txt','out file 3.txt')
MazeRouter('input file 4.txt','out file 4.txt')
MazeRouter('input file 5.txt','out file 5.txt')
MazeRouter('input file 6.txt','out file 6.txt')
MazeRouter('input file 7.txt','out file 7.txt')
MazeRouter('input file 8.txt','out file 8.txt')

#read from LEF and DEF
generateInput()
